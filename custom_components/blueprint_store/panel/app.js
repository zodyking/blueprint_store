    ...
    if (!expanded) {
      ...
      try{
        if (!detailCache.has(it.id)) {
          ...
          // patch item import_url/uses if backend extracted them
          if (!it.import_url && data.import_url) it.import_url = data.import_url;
          if (data.uses != null && it.uses == null) it.uses = data.uses;
        }
